<template>
  <v-container class="col-12" style="padding: 24px; padding-top: 12px; justify-content: space-between;">
        <v-row>  
          <div class="col-12" style="padding: 12px;">
            <h5 style="color:white;">
              <b>
              <v-icon style="color: white; font-size: 1.2rem; margin-bottom: 0.3rem;">mdi-text-box-search</v-icon> TEXT ANALYSER<span style="color: #44d62c;">.</span>
              </b>
            </h5>
          </div>
        </v-row>
        <v-row class="mb-6">
          <div class="col-12" style="padding: 12px;">
            <v-chip-group dark mandatory color="#44d62c" v-model="section">
              <v-chip value="PDF">
                PDF UPLOAD
              </v-chip>
              <v-chip value="paste">
            ...
            <v-card class="cardShadow displayCardBackground" style="padding: 12px; text-align: center; background-color: #283046;">
              <div class="col-12 d-flex mt-0 mb-0">
                <div v-if="this.section == 'PDF'" class="p-12 d-flex justify-items-center align-items-center" style="width: 100%; border: 2px dashed #44d62c; height: 650px;" @dragover="dragover" @dragleave="dragleave" @drop="drop">
                  <input type="file" name="fields[assetsFieldHandle][]" id="assetsFieldHandle" class="w-px h-px opacity-0 overflow-hidden absolute" @change="onChange" ref="file" accept=".pdf" />
                    <label for="assetsFieldHandle" class="block cursor-pointer">
                      <div  v-if="!this.filelist.length" style="position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%);">
                          <h5>
                          <strong style="color: white;">
                            DRAG & DROP A PDF HERE OR <br>CLICK THE BUTTON BELOW
                          </strong>
                          </h5>
                          <v-btn title="Add image" elevation=0 outlined color="#44d62c" @click='addFilesClicked()'>Add PDF</v-btn>
                      </div>
                    </label>
                ...
                <div v-if="this.section == 'paste'" class="p-12" style="width: 100%; height: 650px;">
                  <p style="text-align: left; font-size: 0.75rem; color: white;"><b>Copy and paste the "Place Data"</b> - Lorem ipsum dolor sit amet, consectetur adipisicing elit. Non ea deleniti quibusdam suscipit vel itaque minima, officiis, quo soluta numquam quasi sed adipisci ad quam, nemo minus dicta quia recusandae!</p>
                  <v-textarea dark outlined v-model="placeData" height=540px></v-textarea>
                  <div class="d-flex p-0 justify-content-end"> 
                    <v-btn color="#44d62c" @click="submitMetaData()" elevation="0" title="Analyse Text" class="col-3 no-padding" outlined><span >Analyse Text</span></v-btn>
                  </div>
                </div>
              </div>
            </v-card>
